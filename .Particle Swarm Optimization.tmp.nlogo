...
[
  ...
  personal-mejor-set ; the attributes which make this the best
]
...
globals
[
  global-mejor-set    ; the attributes which make this the best
  global-mejor-val    ; best value found yet
]
...
to set-global-solution[turt]
  ...
  ask turtle turt [
    set global-mejor-set personal-mejor-set  ; the attributes which make this the best
    set global-mejor-val personal-mejor-val   ; best value found yet
...
to-report k-tournamentSelection [N k sol]
  ...
  let elemToAdd one-of elements with [not member? (element who) sol]
  repeat N [
    repeat k [
      ask one-of elements with [not member? (element who) bestSol][ ;randomly choose one element which is not already in the solution
